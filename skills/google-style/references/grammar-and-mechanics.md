...
- Second person throughout. "We" only for a recommendation from the product team: "We recommend that you enable versioning." Never "I."
...
- **Colon** introduces a list, a definition, or an example. The text before it must be a complete sentence.
...
- Use a space between a number and its unit (`512 MB`), except for degrees and percentages in dense reference material.
...
- Times: 12-hour clock with `a.m.`/`p.m.` and an explicit time zone, or ISO 8601 in technical contexts.
